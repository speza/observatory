...
// Product marks retain their supplied brand colours and path geometry.
// Monochrome marks switch only between their official black and white
// treatments; goal and state colours cannot recolour them:
  ...
  if (brand === "codex") {
    return (
      <path
        ...
        fill="var(--agent-logo-monochrome, #000000)"
      />
    );
  }
  if (brand === "pi") {
    return (
      <>
        <path
          ...
          fillRule="evenodd"
        />
        <path d="M517.36 400H634.72V634.72H517.36Z" fill="var(--agent-logo-monochrome, #000000)" />
      </>
    );
  }
  return (
...
const viewBoxFor = (brand: AgentBrand): string => {
  // Source canvases have very different clear-space margins. These optical
  // view boxes give every visible mark the same footprint without altering it.
