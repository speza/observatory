...
const sessionRepositoryIds = (node: FixtureNode): string[] =>
  node.repositoryIds ?? (node.repositoryId ? [node.repositoryId] : []);

const isPrimaryNode = (node: FixtureNode): boolean =>
  node.kind === "goal" || node.kind === "session";
...
class DeterministicFixture {
  ...
  readonly infrastructure: FixtureNode[] = [
    fixtureNode(
      ...
      8,
      1,
      "Synthetic repository anchor used by routing sessions.",
      { branch: "dev" },
    ),
    fixtureNode(
      "repo-food",
      ...
      7,
      2,
      "Synthetic clean-room food-agent repository.",
      { branch: "main" },
    ),
    fixtureNode("repo-mason", "repository", "mason", 7, 1, "Synthetic narrow runtime repository.", {
      branch: "main",
    }),
    fixtureNode(
      "repo-ao",
      ...
      5,
      3,
      "Repository used by the disposable rendering experiment.",
      { branch: "experiment" },
    ),
    fixtureNode(
      "wt-router",
      ...
      4,
      1,
      "Shared write-capable worktree for the routing implementation.",
      { repositoryId: "repo-frontier", branch: "ao/router-main" },
    ),
    fixtureNode(
      "wt-evals",
      "worktree",
      ...
      4,
      1,
      "Shared read-heavy evaluation worktree with overlapping files.",
      { repositoryId: "repo-frontier", branch: "ao/router-evals" },
    ),
    fixtureNode(
      "wt-checkout",
      "worktree",
      ...
      4,
      2,
      "Shared browser proof worktree; checkout remains human-controlled.",
      { repositoryId: "repo-food", branch: "food/basket-proof" },
    ),
    fixtureNode(
      "wt-mason",
      "worktree",
      ...
      4,
      1,
      "Shared runtime and artifact-check worktree.",
      { repositoryId: "repo-mason", branch: "mason/narrow-loop" },
    ),
    fixtureNode(
      "wt-spike",
      "worktree",
      ...
      3,
      3,
      "Disposable native-terminal rendering worktree.",
      { repositoryId: "repo-ao", branch: "experiment/opentui" },
    ),
  ];
  ...
  private buildDeterministicLayout(): Map<string, Point> {
    ...
    for (const goal of goals) {
      ...
      const sessions = this.nodes.filter(
        (node) => node.kind === "session" && node.goalId === goal.id,
      );
      ...
      for (const [index, session] of sessions.entries()) {
        ...
        positions.set(session.id, {
          x: clamp(
            Math.round(goalPoint.x + Math.cos(angle) * orbitRadiusX),
            goalPoint.x - 14,
            goalPoint.x + 14,
          ),
    ...
    for (const [index, repository] of repositories.entries()) {
      const relatedSessions = this.nodes.filter(
        (session) =>
          session.kind === "session" && sessionRepositoryIds(session).includes(repository.id),
      );
      const relatedXs = relatedSessions
        .map((session) => positions.get(session.id)?.x)
        .filter((x): x is number => x !== undefined);
      const fallbackX = centeredLayoutPositions(50, repositories.length, 24)[index] ?? 90;
      const repositoryX =
        relatedXs.length > 0
          ? relatedXs.reduce((total, x) => total + x, 0) / relatedXs.length
          : Math.max(78, fallbackX);
      positions.set(repository.id, { x: clamp(repositoryX, 8, 92), y: 32 });

      const worktrees = this.infrastructure.filter(
        (node) => node.kind === "worktree" && node.repositoryId === repository.id,
      );
    ...
    for (const [index, node] of allNodes.entries()) {
      if (!positions.has(node.id))
        positions.set(node.id, { x: fallbackPositions[index] ?? 50, y: 20 });
    }
  ...
  tick(): void {
    ...
    for (const [index, node] of this.nodes.entries()) {
      ...
      if (node.kind === "session") {
        ...
        node.activeAgents =
          node.id === "session-chief" || node.id === "session-mason"
            ? 2
            : activityWave > -0.35
              ? 1
              : 0;
  ...
  private recomputeAggregates(): void {
    ...
    for (const node of this.nodes) {
      if (node.kind === "goal") {
        const sessions = this.nodes.filter(
          (child) => child.kind === "session" && child.goalId === node.id,
        );
        ...
        node.status = sessions.some((child) => child.status === "blocked")
          ? "blocked"
          : node.attention
            ? "waiting"
            : "active";
    ...
    for (const goal of this.nodes.filter((node) => node.kind === "goal")) {
      ...
      goal.infrastructureWarningText =
        counts.conflicts > 0
    ...
    for (const node of this.infrastructure) {
      const sessions =
        node.kind === "repository"
          ? this.nodes.filter(
              (child) => child.kind === "session" && sessionRepositoryIds(child).includes(node.id),
            )
...
const centeredLayoutPositions = (
  ...
  return Array.from({ length: count }, (_, index) =>
    clamp(start + index * spacing, minimum, maximum),
  );
};
...
class UniverseApp {
  ...
  private renderFrame(deltaTime: number): void {
    ...
    this.currentPortableFocus =
      this.state.mode === 1 && this.state.focusOnly
        ? this.buildPortableFocus()
  ...
  private toggleRepositoryTopology(): void {
    ...
    if (!this.state.repositoryTopology && selected && !isPrimaryNode(selected)) {
      const relatedSession = this.fixture.nodes.find(
        (node) =>
          node.kind === "session" &&
          (selected.kind === "worktree"
            ? node.worktreeId === selected.id
            : sessionRepositoryIds(node).includes(selected.id)),
      );
  ...
  private getViewport(width: number, height: number): Viewport & { inspector?: Rect } {
    ...
    const map = {
      x: 0,
      y: headerHeight,
      width: Math.max(20, width - inspectorWidth),
      height: mapHeight,
    };
    return {
      ...map,
  ...
  private drawHeader(buffer: OptimizedBuffer, width: number): void {
    ...
    const leftHint = compact
      ? "1/2/3 modes  j/k select  v infra  f lens  / find"
      : "1 portable  2 orbital  3 enhanced  v infra lens  f lens";
    ...
    this.textRight(
      buffer,
      shorten(diagnostic, Math.max(10, width - leftHint.length - 4)),
      width - 1,
      1,
      COLORS.textDim,
      COLORS.bg,
    ...
    this.text(
      ...
      1,
      2,
      this.state.suspended ? COLORS.attentionHot : COLORS.textFaint,
      COLORS.bg,
    );
    this.textRight(
      buffer,
      ...
      width - 1,
      2,
      this.state.searchActive ? COLORS.attention : COLORS.textFaint,
      COLORS.bg,
    );
  }
  ...
  private drawMap(buffer: OptimizedBuffer, viewport: Viewport): void {
    ...
    const positions = new Map<string, Point>();
    for (const node of visibleNodes)
      positions.set(node.id, this.worldToScreen(node, viewport.inner));
    ...
    for (const node of visibleNodes) {
      ...
      this.hitTargets.push({
        ...
        x: point.x,
        y: point.y,
        radiusX: radii.radiusX + 1,
        radiusY: radii.radiusY + 1,
      });
    ...
    for (const node of visibleNodes) {
      ...
      const radii = this.nodeRadii(node);
      if (this.inRect(point, viewport.inner, radii.radiusX + 1, radii.radiusY + 1))
        this.drawCalmLabel(buffer, node, point, radii);
    }
  }
  ...
  private drawPortableBackground(buffer: OptimizedBuffer, rect: Rect): void {
    ...
    const lens = this.state.focusOnly
      ? "selected path bright · f for full topology"
      : "full topology · f for selected path";
    const infrastructure = this.state.repositoryTopology
      ? " · repository topology lens"
      : " · infrastructure hidden";
    this.text(
      buffer,
      `PORTABLE OBSERVATORY  ·  direct goal/session  ·  ${lens}${infrastructure}`,
      rect.x + 2,
      rect.y,
      COLORS.cyan,
      COLORS.bgMap,
  ...
  private drawOrbitalBackground(buffer: OptimizedBuffer, rect: Rect): void {
    ...
    for (let i = 0; i < 16; i += 1) {
      ...
      this.cell(
        ...
        y,
        i % 4 === 0 ? "✦" : "·",
        i % 4 === 0 ? COLORS.blue : COLORS.textFaint,
        COLORS.bgMap,
      );
    }
    const infrastructure = this.state.repositoryTopology
    ...
    this.text(
      buffer,
      `ORBITAL OBSERVATORY  ·  direct goal/session orbits + frame-buffer motion${infrastructure}`,
      rect.x + 2,
      rect.y,
      COLORS.violet,
      COLORS.bgMap,
  ...
  private drawEnhancedBackground(buffer: OptimizedBuffer, rect: Rect): void {
    ...
    for (let y = 0; y < sourceHeight; y += 1) {
      for (let x = 0; x < sourceWidth; x += 1) {
        ...
        const wave =
          Math.sin(nx * 11 + this.state.phase * 0.45) * 0.15 +
          Math.cos(ny * 15 - this.state.phase * 0.3) * 0.12;
    ...
    this.text(
      buffer,
      `ENHANCED OBSERVATORY  ·  direct goal/session + native supersampled buffer${infrastructure}`,
      rect.x + 2,
      rect.y,
      COLORS.magenta,
      COLORS.bgMap,
  ...
  private drawRelations(buffer: OptimizedBuffer, rect: Rect, positions: Map<string, Point>): void {
    for (const relation of this.fixture.relations) {
      ...
      const selectedRelation =
        relation.from === this.state.selectedId || relation.to === this.state.selectedId;
      const focused =
        this.state.mode === 1 && this.state.focusOnly
          ? selectedRelation
          : this.currentPortableFocus.has(relation.from) &&
            this.currentPortableFocus.has(relation.to);
      ...
      if (
        ...
        this.cell(
          ...
          marker.y,
          markerGlyph,
          focused ? markerColor : COLORS.textFaint,
          COLORS.bgMap,
        );
      }
    }
  ...
  private drawGoalOrbits(buffer: OptimizedBuffer, rect: Rect, positions: Map<string, Point>): void {
    ...
    for (const goal of goals) {
      ...
      for (let step = 0; step < 36; step += 1) {
        ...
        if (step % 2 === 0)
          this.cell(buffer, x, y, "·", priorityColor(goal.priority), COLORS.bgMap);
      }
    }
  }

  private drawPortableNode(
  ...
  private drawCalmNode(
    ...
    const focused =
      this.state.mode !== 1 || !this.state.focusOnly || this.currentPortableFocus.has(node.id);
    const quiet =
      node.kind === "session" &&
      this.state.mode === 1 &&
      this.state.focusOnly &&
    ...
    const strongAttention =
      node.status === "blocked" || node.infrastructureWarningText?.includes("conflict") === true;
    const pulse = node.attention
      ? Math.sin(this.state.phase * 2.2 + stableNumber(node.id)) > 0
        ? strongAttention
          ...
          : COLORS.attention
        : strongAttention
          ? this.mixWithBackground(COLORS.attentionHot, 0.5)
          : this.mixWithBackground(COLORS.attention, 0.5)
      : priority;
    const priorityRing = quiet
      ? this.mixWithBackground(priority, 0.28)
      : this.mixWithBackground(priority, 0.68);
    const ring = selected
      ? COLORS.white
      : node.attention
        ? pulse
        : node.infrastructureWarning
          ? node.infrastructureWarningText?.includes("conflict")
            ? COLORS.attentionHot
            : COLORS.attention
          : priorityRing;
    ...
    if (node.kind === "goal") {
      ...
      this.cell(
        ...
        marker,
        cardFill,
        selected || node.attention ? TextAttributes.BOLD : TextAttributes.NONE,
      );
    }

    if (node.activeAgents > 0) {
      const activityColor =
        selected || this.state.hoveredId === node.id
          ? COLORS.green
          : quiet
            ? COLORS.textFaint
  ...
  private drawCalmLabel(
    ...
    const quiet =
      ...
      !node.attention;
    const forceLabel =
      this.state.selectedId === node.id || focused || node.kind === "goal" || node.attention;
    this.drawNodeLabel(
      buffer,
      node,
      point,
  ...
  private drawNodeLabel(
    ...
    const leftX =
      preferredX + estimatedWidth < rightEdge
        ? preferredX
        : point.x - radii.radiusX - estimatedWidth - 2;
    ...
    const candidates = (
      node.kind === "goal"
        ? [...centeredCandidates, ...sideCandidates]
        : [...sideCandidates, ...centeredCandidates]
    ).map((candidate) => ({
    ...
    const chosen = candidates.find((candidate) => {
      ...
      const overlapsLabel = this.labelRects.some((existing) =>
        this.rectanglesOverlap(labelRect, existing, 1),
      );
      const overlapsNode = this.currentNodeBounds.some(
        ({ id, rect }) => id !== node.id && this.rectanglesOverlap(labelRect, rect),
      );
      return !overlapsLabel && !overlapsNode;
    });
    if (!chosen && this.state.selectedId !== node.id && node.kind !== "goal" && !node.attention)
      return;
    ...
    this.text(
      ...
      foreground,
      background,
      node.kind === "goal" || hovered ? TextAttributes.BOLD : TextAttributes.NONE,
    );
  ...
  private drawInspector(buffer: OptimizedBuffer, rect: Rect): void {
    ...
    if (node.kind === "session")
      write(
        `tracked session · ${node.status === "idle" || node.status === "done" ? "stopped / resumable" : "live agent view"}`,
        COLORS.textDim,
      );
    write(
      statusLabel(node),
      node.attention ? COLORS.attentionHot : node.status === "blocked" ? COLORS.red : COLORS.green,
      TextAttributes.BOLD,
    );
    ...
    if (node.infrastructureWarning)
      writeWrapped(
        `EXECUTION WARNING  ${node.infrastructureWarningText ?? "shared execution infrastructure"}`,
        COLORS.attentionHot,
      );
    y += 1;
    ...
    if (node.kind === "session") {
      const repositories = sessionRepositoryIds(node).map(
        (repositoryId) => this.fixture.get(repositoryId)?.label ?? repositoryId,
      );
      ...
      if (node.worktreeId) {
        ...
        const shared = this.fixture.nodes.some(
          (other) =>
            other.kind === "session" &&
            other.id !== node.id &&
            other.worktreeId === node.worktreeId,
        );
        write(
          `worktree ${worktree?.label ?? node.worktreeId}${shared ? " · shared" : " · isolated"}`,
          node.infrastructureWarning ? COLORS.attention : shared ? COLORS.yellow : COLORS.textDim,
        );
      }
      ...
      if (node.kind === "worktree" && node.repositoryId)
        write(
          `repository ${this.fixture.get(node.repositoryId)?.label ?? node.repositoryId}`,
          COLORS.blue,
        );
      const attachedSessions = this.fixture.nodes.filter(
        (session) =>
          session.kind === "session" &&
          (node.kind === "worktree"
            ? session.worktreeId === node.id
            : sessionRepositoryIds(session).includes(node.id)),
      );
      if (attachedSessions.length > 0)
        write(
          `sessions ${attachedSessions.map((session) => session.label).join(", ")}`,
          COLORS.textDim,
        );
    }
    const outgoing = this.fixture.relations
      .filter((relation) => relation.from === node.id)
      .map(
        (relation) =>
          `${relationLabel(relation.type)} → ${this.fixture.get(relation.to)?.label ?? relation.to}`,
      );
    const incoming = this.fixture.relations
      .filter((relation) => relation.to === node.id)
      .map(
        (relation) =>
          `← ${this.fixture.get(relation.from)?.label ?? relation.from} ${relationLabel(relation.type)}`,
      );
  ...
  private drawFooter(buffer: OptimizedBuffer, width: number, height: number): void {
    ...
    const controls =
      "1/2/3 mode  j/k select  h/l/u/d pan  +/- or wheel zoom  drag pan  f focus  v infra  / find  i inspector  t diagnostics  s suspend  q quit";
    ...
    this.text(
      buffer,
      shorten(`${query}  selected=${selection?.label ?? "none"}`, Math.max(1, width - 2)),
      1,
      y + 1,
      this.state.searchActive ? COLORS.attention : COLORS.textFaint,
      COLORS.bg,
    ...
    this.textRight(
      buffer,
      `mode ${this.state.mode}  ${this.state.suspended ? "SUSPENDED" : "live"}`,
      width - 1,
      y + 1,
      this.state.suspended ? COLORS.attentionHot : COLORS.green,
      COLORS.bg,
    );
  }
  ...
  private semanticPath(node: FixtureNode): string {
    ...
    if (node.kind === "worktree") {
      if (node.repositoryId)
        labels.push(this.fixture.get(node.repositoryId)?.label ?? node.repositoryId);
      labels.push(`worktree ${node.label}`);
      return labels.join(" / ");
    }
    if (node.goalId) labels.push(this.fixture.get(node.goalId)?.label ?? node.goalId);
    if (node.kind === "session" && node.delegatedBy)
      labels.push(`child of ${this.fixture.get(node.delegatedBy)?.label ?? node.delegatedBy}`);
    const repositories = sessionRepositoryIds(node).map(
      (repositoryId) => this.fixture.get(repositoryId)?.label ?? repositoryId,
    );
  ...
  private capabilitySummary(capabilities: TerminalCapabilities | null): string {
    ...
    const graphics = [
      capabilities.kitty_graphics ? "kitty" : "",
      capabilities.sixel ? "sixel" : "",
      capabilities.rgb ? "rgb" : "ansi",
    ]
      .filter(Boolean)
      .join("+");
  ...
  private enhancedReport(): string {
    ...
    const advertised = capabilities
      ? [capabilities.kitty_graphics ? "Kitty" : "", capabilities.sixel ? "Sixel" : ""]
          .filter(Boolean)
          .join(" + ")
      : "capability query pending";
    if (this.state.enhancedFallback)
      return `${this.state.enhancedFallback}; orbital cell fallback is active.`;
    if (advertised)
      return `${advertised} advertised, but @opentui/core 0.4.5 exposes no public image or @opentui/three renderable here. Native supersampled framebuffer is active; no graphics escape sequence is emitted.`;
  ...
  private handleKey(key: KeyEvent): void {
    ...
    const command =
      key.name === "plus" || key.name === "kpplus"
        ? "+"
        : key.name === "equal" || key.name === "kpequal"
          ? "="
  ...
  private moveSelection(direction: number): void {
    ...
    const index = Math.max(
      0,
      nodes.findIndex((node) => node.id === this.state.selectedId),
    );
  ...
  private applySearch(): void {
    ...
    const found = searchableNodes.find((node) => {
      ...
      const repositories = repositoryIds.flatMap((repositoryId) => [
        repositoryId,
        this.fixture.get(repositoryId)?.label ?? repositoryId,
      ]);
      const worktree = node.worktreeId
        ? [node.worktreeId, this.fixture.get(node.worktreeId)?.label ?? node.worktreeId]
        : [];
      const searchText = [
        node.label,
        node.kind,
  ...
  private handleMouseDown(event: MouseEvent): void {
    ...
    if (!this.currentMapRect || !this.inRect({ x: event.x, y: event.y }, this.currentMapRect))
      return;
  ...
  private handleMouseDrag(event: MouseEvent): void {
    ...
    this.state.panX = clamp(
      this.state.panX - (deltaX * 100) / Math.max(1, this.currentMapRect.width),
      -35,
      35,
    );
    this.state.panY = clamp(
      this.state.panY - (deltaY * 40) / Math.max(1, this.currentMapRect.height),
      -16,
      16,
    );
