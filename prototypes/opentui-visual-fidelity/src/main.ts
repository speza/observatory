...
class FidelitySceneApp {
  ...
  private getViewport(width: number, height: number): Viewport {
    ...
    const map = {
      x: 0,
      y: headerHeight,
      width: Math.max(24, width - inspectorWidth),
      height: mapHeight,
    };
    const inner = {
      x: map.x + 1,
    ...
    const scene = {
      x: inner.x + 1,
      y: inner.y + 1,
      width: Math.max(1, inner.width - 2),
      height: Math.max(1, inner.height - 2),
    };
  ...
  private drawHeader(buffer: OptimizedBuffer, width: number): void {
    ...
    this.text(
      buffer,
      "one goal  →  seven sessions  ·  selected path only  ·  infrastructure is metadata",
      1,
      1,
      COLORS.textMuted,
      COLORS.background,
    ...
    this.textRight(
      buffer,
      shorten(diagnostic, Math.max(10, width - 2)),
      width - 1,
      2,
      COLORS.textFaint,
      COLORS.background,
  ...
  private drawScene(buffer: OptimizedBuffer, viewport: Viewport): void {
    ...
    const pathColor = this.state.focused
      ? COLORS.selectedPath
      : this.mixWithBackground(COLORS.selectedPath, 0.66);
  ...
  private geometry(scene: Rect): SceneGeometry {
    ...
    const goal = {
      x: clamp(
        Math.round(scene.x + scene.width * 0.5 + this.state.panX),
        scene.x + 14,
        scene.x + scene.width - 14,
      ),
      y: clamp(
        Math.round(scene.y + scene.height * 0.51 + this.state.panY),
        scene.y + 7,
        scene.y + scene.height - 7,
      ),
    };
    ...
    const points = ORBIT_ANCHORS.map((anchor) => ({
      x: clamp(
        Math.round(goal.x + anchor.x * orbitRadiusX),
        scene.x + sessionRadiusX + 1,
        scene.x + scene.width - sessionRadiusX - 1,
      ),
      y: clamp(
        Math.round(goal.y + anchor.y * orbitRadiusY),
        scene.y + sessionRadiusY + 1,
        scene.y + scene.height - sessionRadiusY - 1,
      ),
    }));
    return {
      goal,
  ...
  private drawGoal(
    ...
    const label =
      scene.width < 80 || scene.height < 18
        ? "GOAL  ·  model router"
        : shorten(`FOCUSED GOAL  ·  ${GOAL.label}`, Math.max(14, scene.width - 4));
    const labelX = clamp(
      point.x - Math.floor(label.length / 2),
      scene.x,
      Math.max(scene.x, scene.x + scene.width - label.length),
    );
    const labelY = clamp(
      point.y - geometry.goalRadiusY - 2,
      scene.y + 1,
  ...
  private drawSession(
    ...
    const pulse =
      Math.sin(this.state.phase * 2.2 + index) > 0
        ? COLORS.blocked
        : this.mixWithBackground(COLORS.blocked, 0.52);
    const ring = selected
      ? COLORS.selected
      : attention
        ? pulse
        : this.mixWithBackground(baseColor, 0.7);
    this.drawOrb(
      buffer,
      point,
    ...
    const placement = this.findLabelPlacement(
      label,
      point,
      index,
      geometry.sessionRadiusX,
  ...
  private findLabelPlacement(
    ...
    if (Math.abs(anchor.x) > 0.5) {
      candidates.push({
        x: anchor.x > 0 ? point.x + radiusX + 2 : point.x - width - radiusX - 2,
        y: point.y,
      });
      candidates.push({
        x: anchor.x > 0 ? point.x + radiusX + 2 : point.x - width - radiusX - 2,
        y: point.y + (anchor.y > 0 ? 2 : -2),
      });
    } else if (anchor.y < 0) {
    ...
    const fallback = candidates.find((candidate) => {
      ...
      const box = { x: bounded.x - 1, y: bounded.y, width: width + 2, height: 1 };
      return (
        !this.overlaps(box, body) &&
        !this.objectBoxes.some((existing) => this.overlaps(box, existing))
      );
    });
  ...
  private drawInspector(buffer: OptimizedBuffer, rect: Rect): void {
    ...
    this.textRight(
      buffer,
      `${this.state.selectedIndex + 1}/${SESSIONS.length}`,
      right,
      rect.y + 1,
      COLORS.textFaint,
      COLORS.panel,
    ...
    this.text(
      ...
      x,
      rect.y + 3,
      session.status === "blocked" ? COLORS.blocked : COLORS.white,
      COLORS.panel,
      TextAttributes.BOLD,
    );
    this.text(
      buffer,
      `${statusLabel(session.status)}  ·  GOAL`,
      x,
      rect.y + 4,
      statusColor(session.status),
      COLORS.panel,
    ...
    if (compact) {
      ...
      this.text(
        buffer,
        shorten(detail, Math.max(10, rect.width - 4)),
        x,
        rect.y + 5,
        COLORS.metadata,
        COLORS.panel,
      ...
      this.text(
        buffer,
        shorten(
          `${session.runtime}  ·  ${session.host}  ·  ${session.contextSize}`,
      ...
      this.text(
        buffer,
        shorten(session.description, Math.max(10, rect.width - 4)),
        x,
        rect.y + 7,
        COLORS.textMuted,
        COLORS.panel,
    ...
    this.text(
      buffer,
      `goal   ${shorten(GOAL.label, rect.width - 10)}`,
      x,
      line,
      COLORS.goal,
      COLORS.panel,
    ...
    this.text(
      buffer,
      `branch ${shorten(session.branch, rect.width - 10)}`,
      x,
      line,
      COLORS.metadata,
      COLORS.panel,
    ...
    this.text(
      buffer,
      `host   ${session.host}  ·  ${session.contextSize}`,
      x,
      line,
      COLORS.textMuted,
      COLORS.panel,
    ...
    this.text(
      buffer,
      shorten(session.lastEvent, rect.width - 4),
      x,
      line,
      COLORS.textFaint,
      COLORS.panel,
  ...
  private drawFooter(buffer: OptimizedBuffer, width: number, height: number): void {
    ...
    const controls =
      width < 92
        ? "j/k select  enter focus  +/- zoom  i inspect  s suspend  q quit"
        : "j/k select  enter focus  +/- zoom  h/l/u/d pan  i inspector  t diagnostics  r reset  s suspend  q quit";
    this.text(buffer, controls, 1, y + 1, COLORS.textMuted, COLORS.background);
    this.textRight(
      buffer,
      this.state.suspended ? "portable · SUSPENDED" : "portable · disposable",
      width - 1,
      y + 1,
      this.state.suspended ? COLORS.blocked : COLORS.goal,
      COLORS.background,
    );
    this.text(
      buffer,
      shorten(this.state.lastAction, Math.max(10, width - 2)),
      1,
      y + 2,
      COLORS.textFaint,
      COLORS.background,
  ...
  private moveSelection(direction: number): void {
    this.state.selectedIndex =
      (this.state.selectedIndex + direction + SESSIONS.length) % SESSIONS.length;
    this.state.focused = false;
    this.state.inspector = true;
    ...
    for (let row = -radiusY; row <= radiusY; row += 1) {
      ...
      for (let dx = -radiusX - 1; dx <= radiusX + 1; dx += 1) {
        ...
        if (topInside && bottomInside)
          this.cell(buffer, point.x + dx, point.y + row, "▀", topColor, bottomColor);
        else if (topInside)
          this.cell(buffer, point.x + dx, point.y + row, "▀", topColor, background);
        else if (bottomInside)
          this.cell(buffer, point.x + dx, point.y + row, "▄", bottomColor, background);
      }
    }
  ...
  private drawOrbHalo(
    ...
    for (let dy = -outerY; dy <= outerY; dy += 1) {
      for (let dx = -outerX; dx <= outerX; dx += 1) {
        const outer =
          (dx * dx) / Math.max(1, outerX * outerX) + (dy * dy) / Math.max(1, outerY * outerY) <= 1;
        const inner =
          (dx * dx) / Math.max(1, radiusX * radiusX) + (dy * dy) / Math.max(1, radiusY * radiusY) <=
          1;
        if (outer && !inner)
          this.cell(buffer, point.x + dx, point.y + dy, "░", foreground, background);
      }
    }
  }

  private drawCaptionTether(
    buffer: OptimizedBuffer,
    point: Point,
    placement: Point,
    labelWidth: number,
